...
namespace Subway.Player
{
    ...
    public class Movement : MonoBehaviour
    {
        ...
        private void ProcessSwipe()
        {
            ...
            StartCoroutine(SlideRoutine(horizontalInput));
        }

        IEnumerator SlideRoutine(int input)
        {
            _isSliding = true;

            float oldPosX = transform.position.x;
            float newPosX = _currentLine * LineOffsetX;
            float time = 0;

            while (time < _slideTime)
            {
                float currentPos = ClampedLelerp(time, 0f, _slideTime, oldPosX, newPosX);
                _characterController.Move(new Vector2(currentPos - transform.position.x, 0f));
                time += Time.deltaTime;
                yield return null;
            }
        ...
        IEnumerator OldSlideRoutine()
        {
            ...
            // Етижи пасатижи а обнулять кто будет?
            _velocity.x = 0;
            _isSliding = false;
        }
